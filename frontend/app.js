...
const navItems = [
  ...
  { id: 'geo', label: 'Geo intelligence', hint: 'Map & hotspots', icon: 'public' },
  { id: 'analytics', label: 'Analytics', hint: 'Trends & comparisons', icon: 'insights' },
  ...
  { id: 'alerts', label: 'Alerts', hint: 'Live notifications', icon: 'notifications' },
  { id: 'admin', label: 'Admin & access', hint: 'API · roles · audit', icon: 'admin_panel_settings' },
];
...
const state = {
  ...
  mobileNavOpen: false,
  sidebarCollapsed: localStorage.getItem('zv_sidebar_collapsed') === 'true',
  newEntryOpen: false,
  reportOpen: false,
  theme: initialTheme,
  registryQuery: '',
...
function sidebar() {
  return `<aside class="sidebar ${state.mobileNavOpen ? 'open' : ''}" aria-label="Primary navigation"><div class="brand"><div class="brand-coordinates">26.8467°N <b>·</b> 80.9462°E</div><div class="brand-copy"><strong>Zameen Vivaad</strong><small>Field intelligence · live registry</small></div><button class="sidebar-collapse icon-button" data-action="toggle-sidebar" aria-label="${state.sidebarCollapsed ? 'Expand sidebar' : 'Collapse sidebar'}" aria-expanded="${!state.sidebarCollapsed}" title="${state.sidebarCollapsed ? 'Expand sidebar' : 'Collapse sidebar'}">${icon(state.sidebarCollapsed ? 'chevron_right' : 'chevron_left')}</button><button class="close-nav icon-button" data-action="close-nav" aria-label="Close navigation">${icon('close')}</button></div><div class="nav-label">Workspace</div><nav class="sidebar-nav">${navItems.map((item) => `<button class="nav-item ${state.activeTab === item.id ? 'active' : ''}" data-nav="${item.id}" aria-current="${state.activeTab === item.id ? 'page' : 'false'}" title="${item.label}">${icon(item.icon)}<span class="nav-node" aria-hidden="true"></span><span class="nav-copy">${item.label}</span><small class="nav-hint">${item.hint}</small>${state.activeTab === item.id ? '<i></i>' : ''}</button>`).join('')}</nav><div class="sidebar-spacer"></div><section class="sidebar-signal" aria-label="Registry signal"><div class="sidebar-signal-heading"><span>Field signal</span><span class="signal-meter" aria-hidden="true"><i></i><i></i><i></i><i></i></span></div><div class="sidebar-signal-meta"><span class="live-dot"></span><strong>Registry synced</strong><small>Today, 08:45 IST</small></div></section><div class="user-card-row"><button class="user-card" data-action="cycle-account" title="Switch demo account"><div class="avatar">${esc((state.currentUser?.name || 'User').split(' ').map((part) => part[0]).join('').slice(0, 2).toUpperCase())}</div><div class="account-copy"><strong>${esc(state.currentUser?.name || 'Demo account')}</strong><span>${esc(state.currentUser?.role || 'Authenticated user')}</span><span class="demo-badge">Demo mode · sample accounts</span></div><span class="account-more">${icon('more_horiz')}</span></button><button class="account-collapse icon-button" data-action="toggle-sidebar" aria-label="${state.sidebarCollapsed ? 'Expand sidebar' : 'Collapse sidebar'}" aria-expanded="${!state.sidebarCollapsed}" title="${state.sidebarCollapsed ? 'Expand sidebar' : 'Collapse sidebar'}">${icon(state.sidebarCollapsed ? 'chevron_right' : 'chevron_left')}</button></div></aside>`;
}

function shell() {
  const activeLabel = navItems.find((item) => item.id === state.activeTab)?.label || '';
  return `<div class="app-shell ${state.sidebarCollapsed ? 'sidebar-collapsed' : ''}">${sidebar()}<div class="main-shell"><header class="topbar"><button class="mobile-menu icon-button" data-action="open-nav">${icon('menu')}</button><div class="crumb"><span>India / Land acquisition</span>${icon('chevron_right')}<strong>${activeLabel}</strong></div><div class="top-actions"><button class="icon-button" aria-label="Search">${icon('search')}</button><button class="notification-button" aria-label="Notifications">${icon('notifications')}<i></i></button><button class="icon-button theme-toggle" aria-label="Toggle color theme" title="Toggle color theme" data-action="toggle-theme">${icon(state.theme === 'dark' ? 'light_mode' : 'dark_mode')}</button><div class="top-date">${icon('calendar_today')} ${new Date().toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</div><span class="backend-status backend-${state.backendStatus}" title="FastAPI connectivity"><i></i>${state.backendStatus === 'online' ? 'ML API online' : state.backendStatus === 'offline' ? 'ML API offline' : 'Checking ML API'}</span></div></header><main id="main" class="content" tabindex="-1">${state.activeTab === 'dashboard' ? overview() : state.activeTab === 'projects' ? projectDesk() : state.activeTab === 'analysis' ? riskStudio() : state.activeTab === 'registry' ? registry() : state.activeTab === 'archive' ? archive() : window.ZameenViews?.[state.activeTab === 'geo' ? 'geoIntel' : state.activeTab] ? window.ZameenViews[state.activeTab === 'geo' ? 'geoIntel' : state.activeTab]({ state, icon, esc, metric, riskTone, miniMap }) : overview()}</main></div>${state.newEntryOpen ? newProjectModal() : ''}${state.reportOpen ? reportModal() : ''}</div>`;
}
...
function bindEvents() {
  ...
  app.querySelectorAll('[data-action]').forEach((element) => element.addEventListener('click', (event) => { if (element.classList.contains('vanilla-modal-backdrop') && event.target !== element) return; const action = element.dataset.action; if (action === 'open-nav') state.mobileNavOpen = true; if (action === 'close-nav') state.mobileNavOpen = false; if (action === 'toggle-sidebar') { state.sidebarCollapsed = !state.sidebarCollapsed; localStorage.setItem('zv_sidebar_collapsed', String(state.sidebarCollapsed)); } if (action === 'new-project' && state.currentUser?.id !== 'pending') state.newEntryOpen = true; if (action === 'close-new-project') state.newEntryOpen = false; if (action === 'report') state.reportOpen = true; if (action === 'close-report') state.reportOpen = false; if (action === 'download-report') window.print(); if (action === 'mark-alerts-read') state.alertsRead = true; if (action === 'refresh-audit') { window.ZameenApi?.loadAuditLog?.().then((entries) => { state.auditLog = entries; render(); }); return; } if (action === 'toggle-theme') { state.theme = state.theme === 'dark' ? 'light' : 'dark'; localStorage.setItem('zv_theme', state.theme); } if (action === 'cycle-account') { const current = localStorage.getItem('zv_demo_account') || 'state-admin'; const next = demoAccountIds[(demoAccountIds.indexOf(current) + 1) % demoAccountIds.length]; window.ZameenApi.login(next).then((user) => { state.currentUser = user; render(); }); return; } render(); }));
...
render();
showIntro();
window.ZameenApi?.login?.().then((user) => { state.currentUser = user; return Promise.all([window.ZameenApi.loadAssessments(), window.ZameenApi.loadAccuracy(), window.ZameenApi.loadAuditLog?.() || Promise.resolve([])]); }).then(([records, accuracy, auditLog]) => { const remote = records.map(assessmentToProject).filter((project) => !state.projects.some((existing) => existing.id === project.id)); if (remote.length) { state.projects = [...remote, ...state.projects]; state.selectedProject = remote[0]; state.stats.totalProjectsActive += remote.length; } state.accuracy = accuracy; state.auditLog = auditLog; render(); }).catch(() => { state.currentUser = { name: 'Offline demo', role: 'Local fallback mode', id: 'offline' }; render(); });
