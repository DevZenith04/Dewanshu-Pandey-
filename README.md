...
| Path | Purpose |
|---|---|
| `frontend/` | Vanilla dashboard frontend: HTML, CSS, JavaScript, charts, assets, and runtime API client. |
| `frontend/api.js`, `frontend/config.js` | Browser API client, backend-compatible form mapping, runtime URL configuration, and graceful fallback. |
| `backend/app.py` | FastAPI backend exposing `/health`, `/api/features`, `/api/predict`, `/api/assessments`, and `/api/recommend`. |
| `models/risk_classifier.joblib` | Trained risk-category model. |
| `models/delay_regressor.joblib` | Trained delay-probability model. |
| `models/risk_label_encoder.joblib` | Trained risk-label encoder. |
| `backend/requirements.txt` | Python dependencies for the FastAPI service and Streamlit companion. |
| `render.yaml` | Render Blueprint for the API and Streamlit service. |
| `backend/streamlit_app.py` | Optional Streamlit backend UI. |
| `models/data.csv`, `models/metadata.json` | Model/source data and metadata assets. |
