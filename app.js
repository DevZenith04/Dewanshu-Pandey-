...
function render() { applyTheme(); chartInstances.splice(0).forEach((chart) => chart.destroy()); app.innerHTML = shell(); bindEvents(); renderCharts(); const content = app.querySelector('.content'); if (content) { content.classList.remove('view-enter'); requestAnimationFrame(() => content.classList.add('view-enter')); } }
...
function bindEvents() {
  ...
  const search = app.querySelector('[data-project-search], [data-risk-search]'); if (search) search.addEventListener('input', (event) => { state.projectQuery = event.target.value; const cursor = event.target.selectionStart; render(); const next = app.querySelector('[data-project-search], [data-risk-search]'); if (next) { next.focus(); next.setSelectionRange(cursor, cursor); } });
  const parcelSearch = app.querySelector('[data-parcel-search]'); if (parcelSearch) parcelSearch.addEventListener('input', (event) => { state.registryQuery = event.target.value; const cursor = event.target.selectionStart; render(); const next = app.querySelector('[data-parcel-search]'); if (next) { next.focus(); next.setSelectionRange(cursor, cursor); } });
